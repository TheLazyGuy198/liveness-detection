...
class BaseTrainer:
    ...
    def __get_dataset___(self):
        self.Dataset = importlib.import_module('dataset.zip_dataset').__dict__[self.config.DATA.DATASET]
        return self.Dataset
    ...
    def get_dataloader(self):
        ...
        if not self.train_mode:
            ...
            test_dataset = get_dataset_from_list(config.DATA.TEST,
                                                 Dataset,
                                                 test_data_transform,
            ...
            train_dataset = get_dataset_from_list(config.DATA.TRAIN,
                                                  Dataset,
            ...
            val_dataset = get_dataset_from_list(config.DATA.VAL,
                                                Dataset,
                                                test_data_transform,
                                                num_frames=config.DATA.NUM_FRAMES,
                                                root_dir=dataset_dir)
            self.val_data_loader = torch.utils.data.DataLoader(val_dataset,
                                                               batch_size,
                                                               num_workers=num_workers,
                                                               shuffle=False,
                                                               pin_memory=True,
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            logging.info('\nEpoch: {}/{} - LR: {:.6f}'.format(epoch, self.epochs, self.init_lr))
            ...
            with tqdm(total=num_train) as pbar:
                for i, batch_data in enumerate(train_data_loader):
                    loss = self._train_one_batch(batch_data=batch_data, optimizer=self.optimizer)

                    pbar.set_description((" total loss={:.3f} ".format(loss.item())))
    ...
    def test(self, test_data_loader):
        ...
        with torch.no_grad():
            for data in tqdm(test_data_loader):
                ...
                gt_dict, pred_dict = self._collect_scores_from_loader(scores_pred_dict,
                                                                      face_label_gt_dict,
                                                                      target['face_label'].numpy(),
                                                                      pred_score,
                                                                      video_ids)
        test_results = {
            'scores_gt': gt_dict,
    ...
    def load_checkpoint(self, ckpt_path):
        ...
        logging.info("[*] Loaded {} checkpoint @ epoch {}".format(ckpt_path, ckpt['epoch']))
